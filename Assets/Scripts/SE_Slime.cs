...
public class SE_Slime : MonoBehaviour
{
    ...
    List<Rigidbody2D> _slimeBodyList = new();
    ...
    void HandleCollision(Collider2D col)
    {
        ...
        if (colBody != null)
        {
            ...
            var slime = colBody.GetComponentInChildren<SE_Slime>();
            if(slime != null)
            {
                _slimeBodyList.Add(colBody);
            }
        }
    ...
    private void FixedUpdate()
    {
        ...
        {
            bool isSlime = _slimeBodyList.Contains(colBody);
            var drag = (isSlime ? 0.75f : 1f) * Drag;
            colBody.linearVelocity *= (1f - drag * Time.fixedDeltaTime);
            colBody.linearVelocity += (isSlime? 0.25f : 1f) * Inheritance * _rb.linearVelocity;
        });
    }
