using ManaSprite.EasyPooling;
using UnityEngine;

public class SIE_Spawn : SIE, IPoolable
{
    [SerializeField] SkillInstance.SkillInstanceState SpawnState = SkillInstance.SkillInstanceState.End;
    [SerializeField] GameObject _spawnPrefab;
    bool _didSpawn;

    float _speed => _skillInstance.Skill.Stats.Speed.Value;

    public void Initialize()
    {
        _didSpawn = false;
    }

    private void Update()
    {
        if (_skillInstance.State != SpawnState) return;
        if (_didSpawn) return;

        _didSpawn = true;
        DoSpawn();
    }

    void DoSpawn()
    {
        var spawn = _spawnPrefab.PooledInstantiate(null, transform.position + transform.up * 0.25f);

        // Inheritance
        // - skillPart
        var skillPart = spawn.GetComponent<SkillPart>();
        if(skillPart != null)
        {
            skillPart.SetSourceSkillInstance(_skillInstance);
        }
        // - skillInstance
        var skillInstance = spawn.GetComponent<SkillInstance>();
        if(skillInstance != null)
        {
            skillInstance.Link(_skillInstance.Skill);
            skillInstance.ParentBody = GetComponent<Rigidbody2D>();
        }

        var spawnBody = spawn.GetComponent<Rigidbody2D>();
        if (spawnBody != null)
        {
            spawnBody.linearVelocity = 0.25f * _speed * transform.up;
        }

        spawn.SetActive(true);
    }
}
